...
6. S9: for each accepted BUG, request `repair_readback` → `fixing` transition and dispatch the repair Builder via `agent-dispatch` (plan_checkpoint: PLAN_REPORT, then continuous execution), who reads `BUG → TASK → CONTRACTS → REQ → DESIGN → RULES`.
