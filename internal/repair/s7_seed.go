...
func createS7ReviewPlanSeed(root string, round, baselineGeneration int, impact ChangeImpact, contractRef ContractRef, taskIDs []string, impactPath string) (ArtifactRef, error) {
	...
	claims := []any{
		...
		map[string]any{"claim_id": "claim-s9-e2e-na", "lens": "e2e", "target": "post-repair user surface", "assertion": "E2E applicability is decided by the complete S7 plan", "oracle": "the S7 Planner either creates behavior Claims or records an evidence-backed N/A decision", "method": "impact analysis", "applicability": "not_applicable", "na_rationale": "S9 creates a seed only; S7 must decide E2E applicability from the affected surface", "na_checklist_id": impactPath, "source_refs": []string{impactPath}},
	}
	assignments := []any{
