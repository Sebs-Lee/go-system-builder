...
> 层：第三层 ｜ 上游：L2 §S3 + L2 全局规则「单一验证分母」 ｜ 版本 v4.0.4（v4.0.3  实施完成 6/6+E2E；v4.0.1 设计对抗审查处置：P0×1+P1×7；v4.0.0 联合审查版：契约流水线断点地图+三查+documents 登记落地；v3.1.0 增 §6；机制事实经调查核实，含 file:line）
...
| 2026-08-17 | v4.0.4 | **对抗审查处置**：CASE 分母权威改判——scenario-model.json 的 branch case_id 是权威宇宙，cases.json 为生成物；model↔cases 不一致显红（删 CASE+删引用的篡改无法再缩小分母，测试钉死 TestContractsReverseClosureUsesModelAsAuthority） | 第三方对抗审查（sub-agent）：生成物可篡改 |
