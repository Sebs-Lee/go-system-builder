...
> 层结构（以第一层 §六 的唯一权威定义为准）：第一层 = 工程哲学与设计蓝图；**本文 = 第二层 = 生命周期整体规划（实战目标层）**；第三层 = 各 Stage 详细设计（L3-S0…L3-S11，含工具承载）；第四层 = 实现规格；第五层 = 实现；第六层 = 实战运营回灌。
...
| 债务登记 | 每个周期验收时登记已知妥协（类型/影响/成本/负责人）；债可累积但必须可视、可排期 | 技术债管理 |
