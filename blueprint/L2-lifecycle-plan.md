...
| 维度 | 内容 |
|:--|:--|
| 任务 | 把契约拆为单职责、可独立验证的任务：读序、允许/禁止路径、完成判据（可执行的收尾契约）、依赖关系，并交付整体派发计划（并行波次与必要串行理由） |
| 把控 | **单职责与可验证性**——每个任务必须有可判定的完成判据（声明"什么证据存在即算完成"，含修复前失败/修复后通过的测试要求）；写入或资源冲突必须有明确归属、隔离或串行处置，共同只读输入不制造假依赖；依赖图无环（D4：收尾契约字段逼出"完成"的可判定定义） |
...
| 入口 | 契约出口达成 |
| 出口 | TASK 与整体派发计划已提交；依赖及必要资源顺序无环、条款覆盖闭合、收尾契约齐备、计划与当前 REQ 任务集合一致；并行语义交 S5 独立审查 |
| 失败路由 | 发现契约缺口 → 回 S3 |
...
| 维度 | 内容 |
|:--|:--|
| 任务 | 独立审查整条规格链：需求↔设计↔契约↔场景的一致性 + 任务批次及整体派发计划的可执行性；通过后原子锁定计划与执行批次 |
...
| 维度 | 内容 |
|:--|:--|
| 任务 | 消费锁定计划持续并行派发：真实前置回收验证后及时释放下游，专职构建者完成各自任务、运行自有测试、如实报告 |
...
全局交付与来源规则：每一阶段的主要 Markdown、代码和测试产出及时提交；未提交内容不能支撑下一阶段资格。上游输入契约逐项选择 Git tree、磁盘 evidence 或 Runtime 快照；具体读取与事务规则由 [L4 状态机核心 §5.4](L4-state-transition-core.md#54-上游输入契约与文件视图)定义。

REQ 在 S1 显式绑定开发主分支与最终发布上游；worktree 从开发分支已提交基线创建，子分支提交后以合并提交回到项目根目录，验证接收后清理。未回收、分支偏离和资源积压只提醒 Agent；必要阶段产出尚未进入声明的 Git tree 时阶段条件仍不满足。权威机制见 [L4 Worktree](L4-worktree-governance.md)。
...
确立已提交阶段交付、上游来源契约与 REQ 分支/临时 worktree 全局规则。 依据：[Worktree / Hook 缺陷报告](../L4-worktree-hook-remediation.md)。


## 共享模型与合同依赖（2026-09-19）

S3 的共同数据模型与 SYNC 协议按业务切片先收敛，再派生 FE/BE 责任；S4 以真实产物依赖拆分任务，S5 冻结共同设计闭包，S6/S7 验证实际同源消费。 机制权威见 [L4 共享模型与合同治理](L4-shared-model-contract-governance.md)。


## S4—S6 派发计划契约（2026-09-19）

S4 交付、S5 审签、S6 持续消费同一整体派发计划。波次不是全局屏障；容量不足只排队，不改变依赖或删减任务。静态计划与实时执行投影分离，机制权威见 [L4 整体派发计划](L4-agent-dispatch-governance.md#dispatch-plan)。本修订为目标设计，当前实现边界见各 L3 当前事实节。
