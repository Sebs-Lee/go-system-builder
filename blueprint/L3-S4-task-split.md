...
> 层：第三层 ｜ 上游：L2 §S4 ｜ 版本 v4.0.0（v3.1.0 + 联合调查核实 + 设计定稿；owner 已拍板，见 §3）
...
| 模板契约校验（机器） | TASK 模板必含字段检查（Team manifest/Assignment ID/Document Manifest/Delivered clauses/Module Impact/Selected Skills/Lifecycle Evidence/Closing Contract）+ 禁含旧机制词 | migration/templates.go:68-79 |
...
| 看板模板 | 任务矩阵（任务/契约来源/负责人/预估/依赖/状态/阻塞）+ 关键路径文本声明——**进程状态的唯一居所**（文档 Status 不再承载过程词汇） | `docs/tasks/index-template.md` |
...
| 4 | DAG 无环零机检：§8 依赖表全库无解析；但 team/validator.go:168-220 已有 DFS 环检测+依赖存在性先例（team manifest 的 depends_on）——可移植 | internal/team/validator.go:168-220 | P1 |
...
**重新裁定不算断点**：TR-003 后 TASK 文件不入 LockedArtifacts 是**正确设计**——S6 执行期要写 TASK 文档（§9 生命周期证据），硬保护会挡 S6；版本锁定由 documents[].sha256 对账承载（改动=validate 红）。索引"派生 TASK"列与任务覆盖声明双居所必漂移——v4 删列（见 §3 D3）。
...
| 怎么定义"完成" | **保持** §7 四行 assert 文本块——判据先于实现 | criterion_id 结构化是 REQ-040/041 演进方向，不抢跑 |
...
7. **登记推进**：TR-002 双 guard（planning_complete+tasks_checked）→ action `register_planning_tasks` 把批次登记进 documents[]（author_agent_id/registered_at/sha256）→ S5。
...
**批次 2 检查与接线层**
- [ ] internal/semantic/tasks.go：TasksCheck（批完整性：TASK-*.md 全 complete、cancelled 跳过注记、≥1 存在；主契约存在：docs/contracts/{id} 文件在；覆盖双向：索引 cell 宇宙 ↔ 任务 §3 声明，缺向指名 cell/任务、幽灵条款指名；DAG：§8 TASK- 引用存在+全批 DFS 无环报环路径）
- [ ] internal/cli/tasks_check.go：`tasks check --root [--json]`（输出 tasks/cancelled/clauses_total/clauses_covered/problems）
- [ ] guards.go：`tasks_checked` guard（挂 TR-002，root 取 state）；`planning_complete` 重写（契约=当前代 documents[] contract+locked 磁盘一致、无则指路 PTR-PLAN-02；任务=磁盘全批 complete；删双分支与文件名回退）
- [ ] actions.go：`register_planning_tasks`（registerDocumentsFromDisk 复用，prefixes=["TASK-"]，wantStatus=complete）；registerDocumentsFromDisk 增 cancelled 跳过语义（TR-003 同享）；删 atomically_lock_execution_batch（registry+实现+注释）
- [ ] loop-definition.json TR-002：guards=["planning_complete","tasks_checked"]，actions=["register_planning_tasks"]
- [ ] guard_specs.go：补 `tasks_checked` + `contracts_checked`（S3 遗留）词条
- [ ] 回归面：engine_test.go:175-186,219,400-420（TR-002 夹具改 documents[] 形态）、task039_01_planning_test.go 14 例、req039 planning_chain 夹具、conformance 单向确认
...
- [ ] blueprint 同步：L3-S5:35 占位行+§55 已解决两缺口+§6.1#1（小版本）；L3-S3 涉占位的历史叙述核对；L2 任务状态词汇表述 grep
- [ ] docs/loop-harness.md：tasks check 命令 + TR-002 变更

**批次 4 E2E 与真度清零**
- [ ] TestS4TaskSplitPipelineE2E：绿路径（索引宇宙+条款清单+依赖链）→ 断链红（幽灵条款/漏覆盖 cell/缺依赖 TASK/双任务互依环/批次夹 draft 各指名）→ TR-002 双 guard 过+action 登记（author/sha 对磁盘）→ cancelled 跳过 → TR-003 幂等重锁（同代替换不堆叠）
- [ ] 全库 grep activated/working/reported/stale 叙述同步；validate --all 绿；真度表全部划线
