...
> 层：第三层 ｜ 上游：L2 §S8 ｜ 版本 v3.1.0（v3.0.0 叙事版 + §6 注意力预算；机制事实经调查核实——现状与 REQ-041 设计态分开标注）
...
总评：深度纪律的载体分裂为**三份文本**（协议六步 / bug-resolution skill / R-P06 规则）——全系统"同一纪律多载体"最重的 stage；模板字段本身（假设表/根因/边界）已是好逼问，坏在 28 字段必填与罕见混排。判定尺见 L3-README「注意力分配原则」。

### 6.1 当前错配（什么不对、为什么不对）

| # | 错配 | 为什么不对（L1 根据） |
|:--|:--|:--|
| 1 | 深度纪律三份文本：协议六步 + skill 步骤 1-5 + bugfix-review（R-P06）规则 | 违反 L3-README 自己的编排原则（同一职责多机制必须声明主备）；三倍阅读 + 已观测漂移（SKILL 写"fail→increment"，runtime 无实现）；三处讲同一件事必然漂移——这不是假设，已发生 |
| 2 | BUG 模板 28 字段，必填与罕见混排 | 字段过载使逼问失效为扫填——当 agent 学会"快速填完 28 格"，D4 就死了（Eroding Goals + Goodhart 的组合）；REQ-041 已认瘦身 |
| 3 | E2E 覆盖缺口硬步骤（"为什么 E2E 没拦住"）埋在 skill 第 4 步 | 最好的钩子应前置到接收 finding 的第一步——它决定调查方向（先查场景清单再查代码），后置=先走了弯路才被纠正 |

### 6.2 阅读预算（谁在何时读什么）

| 角色 | 进入时必读 | 触发式加载 | 永不需要读（机制承载） |
|:--|:--|:--|:--|
| 调查者 | BUG 模板（假设表/根因/边界字段即逼问）+ bug-resolution skill 流程 | 判 duplicate 时读合并三条件；E2E 类 finding 必过覆盖缺口步骤（`e2e-coverage` 命令输出） | R-P06 全文——六行 Closing Contract 模板应活在 BUG 模板 retest contract 字段的行内注释里；param 门槛规则——不填迁移不了，被拒即知 |
| 主会话（裁决） | 六处置路由一张表 | — | 深查方法论——那是调查者的事（公理二分工） |
