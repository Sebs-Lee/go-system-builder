...
> 层：第三层 ｜ 上游：L2 §S7 ｜ 版本 v3.1.0（v3.0.0 叙事版 + §6 注意力预算；机制事实经调查核实——**现状与 REQ-040 设计态明确分开标注**）
...
总评：守卫与相位已是机器判（干净轮谁算都一个结果），但**最重防线（真交互）停在文本承诺层**——REQ-040 即左移方案。判定尺见 L3-README「注意力分配原则」。

### 6.1 当前错配（什么不对、为什么不对）

| # | 错配 | 为什么不对（L1 根据） |
|:--|:--|:--|
| 1 | 真交互约束（禁 URL 直跳/直写库/直调 API）是协议文本 + e2e-tester 角色定义双写的承诺 | 最重要防线承载在最弱层（C2：文本承诺是被学习绕过的静态规则）；REQ-040 的 trace/HAR 机械核对落地后，agent 只需知道"证据会被机器核"——规范内化于证据格式要求，不再依赖阅读 |
| 2 | 覆盖按职责不按 CASE（CASE 粒度是 REQ-040 设计态） | 职责粒度允许"职责内用例沉默"——与 L2 铁律 1"沉默不是不适用"存在口径缺口 |
| 3 | clean_round 双实现：qualitygate/registry.go:180 自建需求表，未复用 verification 纯函数（guards 与 CLI 两处复用属实，"gate 复用"不成立） | 单一真值被打破——四守卫"谁算都一个结果"的前提被两套实现稀释（D1：权威外置的推论是单一权威） |
| 4 | 三个验证者的角色定义/协议段/skill 存在重复叙述 | 同一纪律多载体未声明主备——三处各读一遍=三倍 token，且漂移必然 |
| 5 | clean_round.go:414 `AffectedByChange` 无生产调用者 | 公理三违例：声称供 Hook policy 用，实际零消费 |

### 6.2 阅读预算（谁在何时读什么）

| 角色 | 进入时必读 | 触发式加载 | 永不需要读（机制承载） |
|:--|:--|:--|:--|
| delivery / qa / e2e 验证者 | **本相位**角色定义 + 本相位 skill | 写证据时读对应报告模板 | 其他相位的方法论（顺序相位制的天然红利）；四守卫定义——CLI 三值输出即结论；干净轮判据——机器判 |
| 主会话 | 相位推进状态（hook 投影） | — | 守卫复算；轮次管理细节 |

### 6.3 整改方向

- **合一**：qualitygate 改调 EvaluateCleanRound 单点（消口径分叉风险）；
- **删减**：删 AffectedByChange；角色-协议去重（角色定义=行为权威，协议只留路由与不变量）；
- **左移（REQ-040 承接，已规划）**：corroboration 佐证种类 + 独立性机器校验、CASE 粒度覆盖计数、trace/HAR 真交互机械核对——把承诺层规范搬进结构层；
- **保持**：顺序相位（前组干净是后组守卫）；发现即分流（TR-008 不等三轮跑完）；targeted 重验不满足任何守卫（铁律 1 的机器化）。
...
| 2026-08-15 | v3.1.0 | 新增 §6 注意力预算与渐进披露；如实修正：clean_round"三处复用"实为两处（qualitygate 未复用纯函数，系本次调查新发现并已入 6.1） | owner 指示：渐进披露、机制承载规范、削减平白叙述 |
