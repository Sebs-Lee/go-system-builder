...
| 层 | 文件 | 内容 | 状态 |
|:--|:--|:--|:--|
| 第一层 | `L1-design-principles.md` | 工程哲学与设计蓝图：场景、物理约束 C1-C5、使命与六项把控、主干设计决策 D1-D7、五公理、失效模式目录、演化协议、权威层结构定义 | ✅ v2.2.0 |
| 第二层 | `L2-lifecycle-plan.md` | 生命周期实战目标：三条铁律 + 能量函数、S0-S11 每阶段（任务/把控/风险对应/理论根据/入口/出口/失败路由）、全局规则、层间校验 | ✅ v1.3.1 |
...
| 第四层 | `L4-*.md`（待建） | 各环节具体设计：架构、契约、模板字段级设计、门禁语义 | ⏳ |
| 第五层 | `L5-*.md`（待建） | 实现层文档 | ⏳ |
