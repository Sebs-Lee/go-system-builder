...
四问的分工：问 1/3/4 纯判断（审查者）；问 2 机器只出参考数字（reference load，不产 problem——v4.4.1 裁定），人判"数字合理但这个任务其实很绕"。
...
|:--|:--|:--|:--|:--|:--|
| 1 | 需求覆盖端到端 | 每条 AC 沿 REQ→条款→TASK→**收尾契约 assert** 走通最后一公里（bridge 只到 CASE；CASE→assert 无人对账） | 恒常 | 部分（机器查到 AC↔CASE） | **并入职责 A**（规格一致性的天然延伸） |
| 2 | NFR 落地追踪 | REQ 非功能表的每行有没有落进契约条款与任务（NFR 最易静默掉队——Eroding Goals 的经典入口） | REQ 有 NFR 行 | **缺失** | **并入职责 A** |
| 3 | 负向与错误路径完备 | 契约错误码表 ↔ 场景包负向分支 ↔ 权限拒绝用例三方对账（REQ 流程表的权限列有没有负向场景） | API 类契约 / REQ 有权限列 | 部分（polarity 在场景包内自洽，跨文档对账缺失） | **并入职责 A** |
| 4 | 可测性前向 | 收尾契约要求的证据在 S7 三角度（DV/QA/E2E）下真的可产出？验收标准本身可判定（"响应快"无指标=不可测） | 恒常 | 部分（B 已查"收尾契约可判定"，未查"S7 能产"） | **并入职责 B**（四问之外第五问） |
| 5 | 迁移与破坏性变更处置 | schema 变更/删改字段：**默认干净断裂，不默认向后兼容**（owner 裁定：非必须不兼容——兼容是必须辩护的技术债）。审查者查的是**决策与登记**而非兼容实现：该变更选了兼容还是断裂？选了兼容的，有没有登记（决策理由/影响面/移除路径+责任人）？破坏性变更的数据迁移任务在不在批里？无登记的兼容 = 无声负债，finding | BE 契约含数据模型变更 | **缺失** | **专项触发**（第三任命或并入 B 按触发） |
| 6 | 外部集成韧性 | 第三方依赖的超时/重试/降级设计 | SYNC 契约或 REQ 声明外部依赖 | **缺失** | **专项触发** |
| 7 | 批次节奏 | DAG 关键路径过长（串行瓶颈）？任务量 vs 并行分派度 | 任务数 > 某直觉值 | 部分（无环已机器查） | **并入 B**（语义连贯问的附加半问） |
| 8 | 风险触发验证就位 | critical profile 的 REQ——S7 的 risk-triggered 验证维度在场景包/角度声明中是否已预留位 | coverage_profile=critical | 部分（S7 侧有机制，S5 不预检） | **专项触发** |
...
落地方式（待 owner 裁决后实施）：#1-#4/#7 并入现有双职责的 SKILL 审查要点（不新增任命——避免职责表膨胀）；#5/#6/#8 做**触发式第三任命**（team-planning 的 risk-triggered responsibilities 概念在 S5 侧的对应物：派活时按触发条件增派专项审查者，或作为 A/B 的附加 checklist 段）；暂缓项入 §7 缺口。
...
| 可执行性审到多深 | 四问（一句话测试/单窗口警示/语义连贯/自包含锚点）；"避免 compact"以**提示词（S4 拆分时）+ 审查要点（S5）**的形态进入流程，机器只出参考数字 | 纸面存在性检查——审不出"builder 拿到干不完"；也否决两类过刚：硬阈值门禁（限制创造力——owner 裁定不取）与 token 精确计数（伪精度） |
