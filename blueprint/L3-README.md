...
## 注意力分配原则（各 L3-Sn §6 的共用判定尺——本节是唯一权威，各 stage 只引用不复述）
...
**载体词表**（§6 表格中使用）：叙述（读）/ 模板（填）/ harness（算）/ hook（拦）/ 人闸（批）。

## 文档骨架（每份 L3-Sn 统一——五问讲清一件事 + 注意力预算）
...
6. **注意力预算与渐进披露**：本 stage 的错配诊断（什么不对、为什么不对——指认 L1 根据）、阅读预算表（谁在何时读什么、什么永不需要读因为机制承载）、整改方向（左移/删减/保持）。判定尺用上方共用原则，不重复叙述。
